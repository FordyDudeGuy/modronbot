...
async def info(ctx):
    await ctx.send('==========================\nHi there'+ ctx.message.author.mention +'!'+ "\nI am a Modron.\nMy sole purpose is to roll dice for you.\n_Commands_\n\n/r *x*d*y* where *x* = no. of dice and *y* = dice sides\nYou can also add a + for more dice to be rolled at the same time or for a modifier \n/adv (or just /a) - Rolls 2d20 for adv./disadv, you can also add a modifier to this too.\n/statgen - I'll roll you six attributes ususing 4d6 drop the lowest method.==========================")    
...
async def r(ctx, *roll,):
  ...
  # This converts the argument (which is a Tuple) to a string with no spaces and separates the string into a list of individual terms that were separated by a + or a -. 
